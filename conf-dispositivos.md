...
## Gestión de energía {#gestion-energia}

Desde OpenBSD/adJ 7.8, el sistema base incluye soporte para suspensión (standby) e
hibernación (suspend-to-disk) en la mayoría de computadores portátiles y en
algunos de escritorio. Estas funciones permiten ahorrar energía y retomar el
trabajo rápidamente.

### Suspensión (standby)

La suspensión mantiene la memoria RAM activa pero reduce el consumo al mínimo.
El sistema puede despertarse presionando cualquier tecla o el botón de encendido
(sin mantenerlo pulsado). Para suspender el equipo ejecute:

        zzz

El sistema entrará en modo de suspensión. Para salir de este modo, pulse una
tecla o el botón de encendido brevemente (menos de 5 segundos). Si mantiene el
botón de encendido durante 10 segundos o más, el equipo se apagará por completo.

### Hibernación (suspend-to-disk)

La hibernación guarda todo el contenido de la memoria RAM en el disco de
intercambio (swap) y luego apaga completamente el equipo, sin consumo de
energía. Al encender de nuevo, el cargador de arranque detecta que el sistema
fue hibernado y restaura el estado exacto en que se dejó. Para hibernar ejecute:

        ZZZ

> **Nota:** El comando `ZZZ` debe escribirse en mayúsculas. Algunos sistemas
...
soporta estas funciones o que falta cargar el controlador adecuado. Revise las
bitácoras del sistema con `dmesg` y asegúrese de que el servicio `apmd` esté
activo (por defecto lo está). Para comprobar:

        doas rcctl check apmd

Si no está activo, habilítelo con:
...
- Páginas del manual: `zzz(8)`, `apmd(8)`, `sysctl(8)`
